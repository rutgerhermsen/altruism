...
contains
  ...
  subroutine set_shorthands()
    ...
    ! estimated steady-state density in absence of costs or altruism
    SS = K_COMPETITION*(P_R - P_D)/P_R
    ! upper bound of maximum population size
    MAX_POP = nint(1.5d0*K_COMPETITION*(N_REAL/RESOLUTION)**2d0)
    ! initial population size chosen
    INIT_TOTPOP = nint(INIT_POP*SS*(N_REAL/RESOLUTION)**2d0)
