...
contains
  ...
  subroutine compute_S_curve(field, o_stats)
    ...
    complex(DP) :: fft_p_field(N)
    complex(DP) :: fft_w_field(N)
    integer :: i

    !print *, "computing Scurve at time", t
    nr_SC_C = nr_SC_C + 1
    ...
    call cfft1f(N, 1, fft_p_field, N, wsave, lensav, work, lenwrk, ier)

    fft_w_field = cmplx(field%w, kind = DP)
    call cfft1f(N, 1, fft_w_field, N, wsave, lensav, work, lenwrk, ier)

    ! calculations for sigma = 0
    S_curve_vals(nr_SC_C, 1, 1) = 0d0
    S_curve_vals(nr_SC_C, 1, 2) = &
    & compute_S_interlocal_0(field%occ, field%p, field%w, o_stats%mean_p, o_stats%mean_w, o_stats%n)
    S_curve_vals(nr_SC_C, 1, 3) = S - S_curve_vals(nr_SC_C, 1, 2)
    S_vals(nr_SC_C) = S

    ! calculations for all other values of sigma
    do i = 2, (NR_SCALES + 1)
      call compute_S_split(i - 1, fft_occ_field, fft_p_field, &
      & fft_w_field, S_curve_vals(nr_SC_C, i, 2:3), o_stats)
    end do

  end subroutine compute_S_curve

  real(DP) function compute_S_interlocal_0(occ, p, w, mean_p, mean_w, n) result (s0)
    integer, intent(in) :: occ(N)
    real(DP), intent(in) :: p(N)
    real(DP), intent(in) :: w(N)
    real(DP), intent(in) :: mean_p
    real(DP), intent(in) :: mean_w
    integer, intent(in) :: n
    real(DP) :: tmp(N)
    real(DP) :: occR(N)

    occR = real(occ, kind = DP)

    tmp = 0d0
    where (occ > 0)
      tmp = (p - occR*mean_p) * (w - occR*mean_w) / occR
    end where

    s0 = pSum(tmp)/real(n, kind = DP)
  ...
  subroutine compute_S_split(kn, fft_occ_field, fft_p_field, fft_w_field, ssplit, o_stats)
    ...
    complex(DP), intent(in)  :: fft_occ_field(N)
    complex(DP), intent(in) :: fft_p_field(N)
    complex(DP), intent(in) :: fft_w_field(N)
    real(DP), intent(out) :: ssplit(2)
    type(org_sts), intent(in) :: o_stats
    complex(DP) :: d_est(N)
    complex(DP) :: p_est_unnorm(N)
    complex(DP) :: w_est_unnorm(N)
    logical :: mymask(N)

    ! use kernel to compute the corresponding kernel density estimate
    d_est = fft_kernels(:, kn)*fft_occ_field*cmplx(N, kind = DP)
    call cfft1b(N, 1, d_est, N, wsave, lensav, work, lenwrk, ier)

    ! compute unnormalized mean phenotype estimate
    p_est_unnorm = fft_kernels(:, kn)*fft_p_field*cmplx(N, kind = DP)
    call cfft1b(N, 1, p_est_unnorm, N, wsave, lensav, work, lenwrk, ier)

    ! compute unnormalized mean fitness estimate
    w_est_unnorm = fft_kernels(:, kn)*fft_w_field*cmplx(N, kind = DP)
    call cfft1b(N, 1, w_est_unnorm, N, wsave, lensav, work, lenwrk, ier)
    ...
    & pack(real((p_est_unnorm-d_est*o_stats%mean_p)/d_est, kind = DP), mymask))/real(o_stats%n, kind = DP)
    ! s local
    ssplit(2) = S - ssplit(1)
  ...
  subroutine compute_contr_Slocal(field)
    ...
    complex(DP) :: fft_p_field(N)
    complex(DP) :: fft_w_field(N)
    complex(DP) :: fft_w_p_field(N)
    complex(DP) :: d_est(N)
    complex(DP) :: p_est_unnorm(N)
    complex(DP) :: w_est_unnorm(N)
    complex(DP) :: w_p_est_unnorm(N)
    ...
    call cfft1f(N, 1, fft_w_p_field, N, wsave, lensav, work, lenwrk, ier)

    ! use kernel to compute the corresponding kernel density estimate
    d_est = fft_kernelLSD*fft_occ_field*cmplx(N, kind = DP)
    call cfft1b(N, 1, d_est, N, wsave, lensav, work, lenwrk, ier)

    ! compute unnormalized mean phenotype estimate
    p_est_unnorm = fft_kernelLSD*fft_p_field*cmplx(N, kind = DP)
    call cfft1b(N, 1, p_est_unnorm, N, wsave, lensav, work, lenwrk, ier)

    ! compute unnormalized mean fitness estimate
    w_est_unnorm = fft_kernelLSD*fft_w_field*cmplx(N, kind = DP)
    call cfft1b(N, 1, w_est_unnorm, N, wsave, lensav, work, lenwrk, ier)

    ! compute unnormalized mean product of fitness and phenotype estimate
    w_p_est_unnorm = fft_kernelLSD*fft_w_p_field*cmplx(N, kind = DP)
    call cfft1b(N, 1, w_p_est_unnorm, N, wsave, lensav, work, lenwrk, ier)
  ...
  subroutine update_w_field()
    ...
    field(py)%w = 0d0  !wipe
    do i = 1, o_stats(py)%n
      field(py)%w(o_list(i, py)%x) = &
      & field(py)%w(o_list(i, py)%x) + o_list(i, py)%w
    end do
  ...
  subroutine update_w_p_field()
    ...
    do i = 1, o_stats(py)%n
      field(py)%w_p(o_list(i, py)%x) = &
      & field(py)%w_p(o_list(i, py)%x) + o_list(i, py)%w*o_list(i, py)%p
    end do
